...
1. Identify the active feature folder in `<SHOP_ROOT>/specs/` (the most recent folder, or as specified above).
2. Read the spec at `<SHOP_ROOT>/specs/<NNN>-<feature-name>/feature.spec.md`.
...
8. Re-validate the spec against `<SHOP_ROOT>/specs/<NNN>-<feature-name>/checklists/requirements.md`. Update checklist pass/fail status.
