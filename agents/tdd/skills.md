...
- Architecture constraints and module boundaries (from `<SHOP_ROOT>/reports/pipeline/<NNN>-<feature-name>/adr.md`)
- `<SHOP_ROOT>/reports/codebase-analysis/TESTABILITY-<id>-<date>.md` (if produced by CodeBase Analyzer — consume characterization test targets and seam candidates before writing tests for migrated modules)
...
4. Create test certification record using `<SHOP_ROOT>/templates/test-certification-template.md`. Write to `<SHOP_ROOT>/reports/pipeline/<NNN>-<feature-name>/test-certification.md`. Include spec ID, version, and hash.
